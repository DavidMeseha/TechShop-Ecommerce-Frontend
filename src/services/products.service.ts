...
export async function homeFeedProducts({ page, limit }: { page: number; limit: number }) {
  return axios
    .get<{ data: IFullProduct[]; pages: Pagination }>("/api/v2/catalog/homefeed", {
      params: { page, limit }
    })
    .then((res) => res.data);
}
...
export async function addReview(productId: string, form: { reviewText: string; rating: number }) {
  return axios.post(`/api/v2/user/addReview/${productId}`, { ...form });
}

export async function getVendors(params: { page: number; limit: number }) {
  return axios
    .get<{ data: IVendor[]; pages: Pagination }>("/api/v2/catalog/discover/vendors", { params })
    .then((res) => res.data);
}

export async function getCategories(params: { page: number }) {
  return axios
    .get<{ data: ICategory[]; pages: Pagination }>("/api/v2/catalog/discover/categories", { params })
    .then((res) => res.data);
}

export async function getTags(params: { page: number; limit: number }) {
  return axios
    .get<{ data: ITag[]; pages: Pagination }>("/api/v2/catalog/discover/tags", { params })
    .then((res) => res.data);
}

export async function getProductsByCateory(categoryId: string, params: { page: number }) {
  return axios
    .get<{ data: IFullProduct[]; pages: Pagination }>(`/api/v2/catalog/Category/products/${categoryId}`, { params })
    .then((res) => res.data);
}

export async function getProductsByTag(tagId: string, params: { page: number }) {
  return axios
    .get<{ data: IFullProduct[]; pages: Pagination }>(`/api/v2/Catalog/tag/products/${tagId}`, { params })
    .then((res) => res.data);
}
