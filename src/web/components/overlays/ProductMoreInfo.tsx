...
function MainLogic({ product }: { product: IFullProduct }) {
  ...
  return (
    <>
      ...
      <div className="mb-4 text-center text-sm text-gray-400">
        {product?.productTags
          ? product?.productTags.map((tag) => (
              <LocalLink className="me-4 hover:underline" dir="ltr" href={`/tag/${tag}`} key={tag}>
                #{tag}
              </LocalLink>
            ))
          : null}
      ...
      <SubmitButton className="w-full bg-primary text-white" isLoading={isPending} onClick={addToCartClickHandle}>
